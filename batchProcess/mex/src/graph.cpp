...
// 								adjVertices[v2].remove(v1);
	...
	double percentageEdge = static_cast<double>(edgesAux.size())/ static_cast<double>(edges.size()); 
	...
	return percentageEdge;
}

bool Graph::writeEdges(string fileName)
{
	std::ofstream inf(fileName.c_str(), std::ios_base::out);
	if(!inf.is_open())	
		return false;
